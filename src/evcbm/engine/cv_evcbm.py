...
from .train_evcbm import train_one_split_evcbm
...
def run_kfold_evcbm(cfg: Dict[str, Any],
                          model_factory: Callable[[int, int], torch.nn.Module],
                          get_transforms_fn: Callable[[bool], Tuple] = get_transforms):
    """
    cfg keys: root_dir, dataset, num_folds, seed, batch_size, num_workers, augment,
              learning_rate, weight_decay, epochs, patience, out_dir,
              fp16(optional), min_delta(optional), lambda_concepts(optional),
              concept_threshold(optional), inner_val_ratio(optional, default 0.2)

    外层 K-fold: train/test
    内层在 train 中再切 inner train/val（用于 early stopping）
    最终 Label Acc 使用外层 test 上的表现，Concept Acc 使用 inner val 上的最佳值
    """
    set_seed(int(cfg["seed"]))
    out_base = os.path.join(cfg["out_dir"], cfg["dataset"], "evcbm")
    os.makedirs(out_base, exist_ok=True)

    # Build full dataset once（不带增强的 transform）
    ...
    for fold, (idx_tr, idx_va) in enumerate(skf.split(np.zeros(len(labels)), labels), start=1):
        ...
        best_label_acc, best_concept_acc, best_state = train_one_split_evcbm(
        ...
        test_concept_bce, test_concept_acc = eval_concept_metrics(model, dl_test, device, threshold=concept_threshold)
        ...
        # confusion matrix / summary 基于 test 集
    ...
    # Label / Concept 都用 TEST 的指标做最终报告
    ...
    out_csv = os.path.join(out_base, "cv_results_evcbm_joint.csv")
