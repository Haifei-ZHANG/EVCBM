...
from .train_cem import train_one_split_cem
...
def run_kfold_cem(cfg: Dict[str, Any],
    ...
    out_base = os.path.join(cfg["out_dir"], cfg["dataset"], "cem")
    os.makedirs(out_base, exist_ok=True)

    # Build full dataset once（用非增强 transform，保证 deterministic）
    ...
    for fold, (idx_tr, idx_va) in enumerate(
        ...
        best_label_acc, best_concept_acc, best_state = train_one_split_cem(
            model, dl_train, dl_val, cfg, device=device
        )
        print(
            f"Fold {fold} Best (VAL) label_acc={best_label_acc:.4f} | "
        ...
        # 混淆矩阵 / summary 基于 test
        save_fold_summary(out_base, fold, y_true, y_pred)

        # 同时记录 val 和 test 指标，便于后续分析
        results.append({
            "fold": fold,
    ...
    out_csv = os.path.join(out_base, "cv_results_cem_joint.csv")
