...
# -*- coding: utf-8 -*-
"""
K-fold CV for EVCBM (sequential).
...
from .train_evcbm_sequential import train_one_split_evcbm_sequential
...
def run_kfold_evcbm_sequential(cfg: Dict[str, Any],
                               model_factory: Callable[[int, int], torch.nn.Module],
                               get_transforms_fn: Callable[[bool], Tuple] = get_transforms):
    set_seed(int(cfg["seed"]))
    out_base = os.path.join(cfg["out_dir"], cfg["dataset"], "evcbm_sequential")
    os.makedirs(out_base, exist_ok=True)

    # 用 non-augment transform 构建完整数据集
    ...
    for fold, (idx_tr, idx_va) in enumerate(skf.split(np.zeros(len(labels)), labels), start=1):
        print(f"\n===== EVCBM-Sequential Fold {fold}/{cfg['num_folds']} =====")
        ...
        # 概念 backbone 是在 CBM-sequential 阶段已经训练好的
        ckpt_path = _concept_ckpt_path_for_fold(cfg, fold)
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(
                f"[EVCBM] Expected concept backbone not found for fold {fold}: {ckpt_path}. "
        ...
        # 在 inner train/val 上训练 + early stopping（返回的是 VAL 上的最佳指标）
        best_label_acc, best_concept_acc, best_state = train_one_split_evcbm_sequential(
        ...
        # summary / confusion matrix 基于 test
    ...
    out_csv = os.path.join(out_base, "cv_results_evcbm_sequential.csv")
    df.to_csv(out_csv, index=False)

    # 最终报告：label / concept 都用 TEST 的指标
